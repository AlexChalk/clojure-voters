...
The `home-routes` handler in the `clojure-voters.routes.home` namespace
...
The `home-page` function will in turn call the `clojure-voters.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/clojure_voters/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `clojure-voters.handler` namespace:
...
Request middleware functions are located under the `clojure-voters.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clojure-voters.dev-middleware` namespace found in
